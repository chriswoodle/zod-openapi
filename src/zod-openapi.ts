...
type AnatineSchemaObject = SchemaObject & {[fragmentName]?: string, hideDefinitions?: string[]};
  ...
  openApiVersion,
    ...
    const reduced = Object.keys(zodRef.shape)
      ...
      .reduce(
        (carry, key) => ({
          ...carry,
          [key]: generateSchema(zodRef.shape[key], options, openApiVersion),
        }),
...
function dropFragmentNames(schemas: AnatineSchemaObject[]) {
...
function parseTransformation({
    ...
    const input = generateSchema(zodRef._def.schema, options, openApiVersion);
  ...
  >): SchemaObject {
    ...
    if (
      ...
      additionalProperties = generateSchema(zodRef._def.catchall, options, openApiVersion);
...
function parseOptional({
    ...
    return merge(
      generateSchema(zodRef.unwrap(), options, openApiVersion),
      zodRef.description ? { description: zodRef.description } : {},
      ...dropFragmentNames(schemas)
    );
  }
  ...
  function parseNullable({
    ...
    const schema = generateSchema(zodRef.unwrap(), options, openApiVersion);
    return merge(
      schema,
  ...
  return merge(
    {
      ...
      ...generateSchema(zodRef._def.innerType, options, openApiVersion),
    },
    zodRef.description ? { description: zodRef.description } : {},
    ...dropFragmentNames(schemas)
  );
  ...
  return merge(
    {
      ...
      items: generateSchema(zodRef.element, options, openApiVersion),
      ...constraints,
    },
    zodRef.description ? { description: zodRef.description } : {},
    ...dropFragmentNames(schemas)
...
function parseIntersection({
  ...
  return merge(
    {
      allOf: [
        generateSchema(zodRef._def.left, options, openApiVersion),
        generateSchema(zodRef._def.right, options, openApiVersion),
      ],
    },
    zodRef.description ? { description: zodRef.description } : {},
    ...dropFragmentNames(schemas)
...
function parseUnion({
  ...
  return merge(
    {
      oneOf: contents.map((schema) => generateSchema(schema, options, openApiVersion)),
    },
    zodRef.description ? { description: zodRef.description } : {},
    ...dropFragmentNames(schemas)
  );
  ...
  return merge(
    {
      ...
      ).map((schema) => generateSchema(schema, options, openApiVersion)),
    },
    zodRef.description ? { description: zodRef.description } : {},
    ...dropFragmentNames(schemas)
  );
...
function parseBranded({
    ...
    return merge(generateSchema(zodRef._def.type, options, openApiVersion), ...schemas);
  }
...
function parsePipeline({
    ...
    return merge(
      generateSchema(options?.useOutput ? zodRef._def.out : zodRef._def.in, options, openApiVersion),
      ...dropFragmentNames(schemas)
    );
  }
...
function getSchemasForZodObject(zodRef: OpenApiZodAny): AnatineSchemaObject[] {
...
function getNameFromSchemas(schemas: AnatineSchemaObject[]): string | undefined {
...
export function generateSchema(
  ...
  try {
    ...
    return catchAllParser({ zodRef, schemas, options, openApiVersion });
  } catch (err) {
    console.error(err);
    return catchAllParser({ zodRef, schemas, options, openApiVersion });
  }
...
export function generateVocabulary(objects: OpenApiZodAny[], openApiVersion: OpenAPIVersion): [{[key: string]: SchemaObject}, Set<string>] {
  ...
  const fragmentSchemas = Object.fromEntries(fragments.map(fragment => {
    ...
    const schema = generateSchema(fragment, { vocabulary: fragmentNames }, openApiVersion) as SchemaObject;
